...
//    const uint8_t* end = (const uint8_t*)nalu + bytes;
	...
	static int naluCnt=0;
	naluCnt++;

	LOG_DBG("nalutype=%d [%3d]\n", nalutype, naluCnt);
}

void _avc2flv_test(const char* inputH264, const char* outputFLV){
...
int avc2flv_test(int argc, char const *argv[]){
	string outFlvFile(g_appCfg.datOutPath + "/out.flv");
	_avc2flv_test(string(g_appCfg.datOutPath + "/video.h264").c_str(), outFlvFile.c_str());
	return 0;
}
