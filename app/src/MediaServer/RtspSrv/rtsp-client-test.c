...
void rtp_receiver_test(socket_t rtp[2], const char* peer, int peerport[2], int payload, const char* encoding, char* recvPath);
...
static int onsetup(void* param, int64_t duration)
{
	...
	for (i = 0; i < rtsp_client_media_count(ctx->rtsp); i++)
	{
		...
		if (RTSP_TRANSPORT_RTP_UDP == transport->transport)
		{
			...
			if (*transport->source)
			{
				rtp_receiver_test(ctx->rtp[i], transport->source, port, payload, encoding, ctx->recvPath);
			}
			else
			{
				socket_getpeername(ctx->socket, ip, &rtspport);
				rtp_receiver_test(ctx->rtp[i], ip, port, payload, encoding, ctx->recvPath);
			}
		}
...
{
	LOG_RTSP_INFO("client on teardown\n");
	return 0;
}
...
{
	LOG_RTSP_INFO("rtsp client on play\n");
	return 0;
}

static int onpause(void* param)
{
	LOG_RTSP_INFO("rtsp client on pause\n");
	return 0;
}

void _rtsp_client_test(const char* _host, const char* file, const char* savPath){
	int r;
	...
	LOG_INFO("start rtsp client %s %s\n", host, file);
	snprintf(ctx.recvPath, sizeof(ctx.recvPath), "./%s/", savPath=NULL?"sub1":savPath);
	int ire = 0;
	if(path_testdir(ctx.recvPath)==0){
		ire = path_makedir(ctx.recvPath);
		if(ire != 0){
			fatal("path makedir err %s\n", ire);
		}
	}
